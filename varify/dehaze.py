...
def dehaze(img_refer,img_corr,windows_size=15):##for gray image
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            t[i,j]=min(abs((high_corr[i,j]-low_corr[i,j])/(high_refer[i,j]-low_refer[i,j])),1)   
            t[i,j]=max(t[i,j],0.0)    
            #t[i,j]=(high_corr[i,j]-low_corr[i,j])/(high_refer[i,j]-low_refer[i,j])
...
if __name__=='__main__':
    mod='selected'
    refer=os.path.join('./essay/varify',mod,'2.tif')
    dir=os.path.join("./essay/varify/haze",mod,"mid")
    #refer='./essay/varify/ex/2.tiff'
    #dir="./essay/varify/ex"
    s_dir="./essay/varify/dehaze/my_e"+mod
